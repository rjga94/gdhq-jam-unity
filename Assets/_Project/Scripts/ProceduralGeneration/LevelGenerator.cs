...
namespace ProceduralGeneration
{
    public class LevelGenerator
    {
        ...
        public LevelGenerator(Tilemap tilemap, TileBase groundTile, int holeWidth, int holeSize, float platformFrequency, int platformMinWidth, int platformMaxWidth, GameObject corePrefab)
        {
            _tilemap = tilemap;
        ...
        public void GenerateTiles()
        {
            ...
            {
                _tilemap.SetTile(new Vector3Int(-halfWidth - 1, -j , 0), _groundTile);
                _tilemap.SetTile(new Vector3Int(halfWidth, -j , 0), _groundTile);

                for (var k = 0; k < 18; k++)
                {
                    _tilemap.SetTile(new Vector3Int((-halfWidth - 1) - k, -j , 0), _groundTile);
                    _tilemap.SetTile(new Vector3Int(halfWidth + k, -j, 0), _groundTile);
                }
                ...
                for (var o = _holeSize + 10; o < _holeSize + 10 + 18; o++)
                {
                    ...
                    {
                        _tilemap.SetTile(new Vector3Int(-k - 1, -o , 0), _groundTile);
                        _tilemap.SetTile(new Vector3Int(k, -o, 0), _groundTile);
                    }
                }
            }
